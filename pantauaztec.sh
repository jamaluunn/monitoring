...
# Telegram bot credentials (ganti dengan token & chat ID kamu)
...
# Cek port 8080 apakah ada aplikasi jalan
...
while true; do
  ...
  message="🔍 Checking Aztec node sync status at $timestamp
🧱 Local block:  $LOCAL
🌐 Remote block: $REMOTE"

  if [[ "$LOCAL" == "N/A" ]] || [[ "$REMOTE" == "N/A" ]]; then
    message="$message
🚫 Cannot determine sync status due to an error in one of the RPC responses."
  elif [ "$LOCAL" = "$REMOTE" ]; then
    message="$message
✅ Your node is fully synced!"
  else
    message="$message
⏳ Still syncing... ($LOCAL / $REMOTE)"
  fi
